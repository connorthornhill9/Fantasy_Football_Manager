...
- Waivers: if the player is a free agent use add or add_drop; if the league uses FAAB, size the bid to the player's value, the competition (adds in the last 24h), and my remaining budget. A player dropped in the last day or two is usually still on waivers, so a waiver_claim is the safer choice for him.
